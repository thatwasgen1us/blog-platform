...
const CreateUser: React.FC = () => {
  ...
  const [registerMutation, { isSuccess, isError, error }] = useRegisterMutation();
  ...
  return (
    <div className={classes.createUser}>
      <form className={classes.createUser__form} onSubmit={handleSubmit(onSubmit)}>
        ...
        <button className={classes.createUser__button} type="submit">Sign Up</button>
        <div className={classes.createUser__link}>
          Already have an account? <Link to="/signin">Sign In.</Link>
        </div>
        {isSuccess && <p style={{color: 'green'}}>User created successfully!</p>}
        {isError && <p style={{color: 'red'}}>Error: {error?.data?.message || 'Такой пользователь уже существует'}</p>}
      </form>
